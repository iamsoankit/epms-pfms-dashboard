...
def load_and_clean_data(url):
    ...
    try:
        ...
        for col in KPI_COUNT_COLS + ['DSO', 'Project_Type', 'Budget', 'Programme_Scheme', 'Division']:
             if col in df.columns:
                # Fill NaNs with a placeholder string to prevent errors in nunique() or grouping
...
if df_filtered.empty:
    ...
    with col_vis2:
        ...
        # --- FIX: Filter out rows where Programme_Scheme is 'N/A' (or blank/null) ---
        scheme_data = df_filtered[df_filtered['Programme_Scheme'].str.strip() != 'N/A'].copy()
        
        if not scheme_data.empty:
            scheme_summary = scheme_data.groupby('Programme_Scheme')['Vetting_Amount_INR'].sum().reset_index()
            ...
            st.info("Pie chart: No Programme/Scheme data available after cleaning.")
